# Sam Ludwig
Melbourne, VIC | 0405 993 245 | sam.ludwig@gmail.com

**Re: Contracts Manager — Techforce Personnel**

Dear Hiring Manager,

I'm writing to express my interest in the Contracts Manager position with Techforce Personnel. I'm writing to express my interest in the Contracts Manager role at Techforce Personnel. My background across enterprise infrastructure, Microsoft 365, and service operations connects well with the requirements outlined.



I bring experience deploying fibre and copper Layer 1 infrastructure across residential and commercial environments.

The areas where my experience aligns most directly with this role are Network.

I'd welcome a conversation about how my background could contribute to your team. I'm happy to confirm any licence, qualification, check, roster or prior-industry requirements before progressing.

Kind regards,
Sam Ludwig

<!-- ✅ Cover letter summary | - Specific listing details referenced: Role: Contracts Manager; Company: Techforce Personnel; Location: Melbourne Region, Melbourne | - Achievements highlighted: Network | - Adjacent-match framing used: no | - Word count: 123 | - Voice guide check: PASS |  -->
